...
const MAX_READER_ATTEMPTS = 2;
const READER_RETRY_DELAY_MS = 2_000;
...
export function getReaderModeMaxAttempts(env: Record<string, string | undefined> = process.env): number {
  const rawValue = env.WORKER_MAX_ATTEMPTS;
  if (!rawValue) return MAX_READER_ATTEMPTS;
  const parsed = Number.parseInt(rawValue, 10);
  if (!Number.isFinite(parsed) || parsed <= 0) return MAX_READER_ATTEMPTS;
  return parsed;
}

export function getReaderModeRetryDelayMs(env: Record<string, string | undefined> = process.env): number {
  const rawValue = env.WORKER_RETRY_BASE_DELAY_MS;
  if (!rawValue) return READER_RETRY_DELAY_MS;
  const parsed = Number.parseInt(rawValue, 10);
  if (!Number.isFinite(parsed) || parsed <= 0) return READER_RETRY_DELAY_MS;
  return parsed;
}
...
export async function extractReaderModeContent(
  ...
  const maxAttempts = toPositiveIntOrDefault(deps.maxAttempts, getReaderModeMaxAttempts());
  const baseDelayMs = toPositiveIntOrDefault(deps.baseDelayMs, getReaderModeRetryDelayMs());
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    if (attempt > 1) {
      ...
      console.warn(`[reader-mode-worker] Retry attempt ${attempt}/${maxAttempts} for link ${payload.linkId} after ${delayMs}ms`, {
...
export async function handleReaderModeMessage(
  ...
  const sleepImpl = deps.sleepImpl ?? sleep;
  const maxAttempts = deps.maxAttempts ?? getReaderModeMaxAttempts();
  const baseDelayMs = deps.baseDelayMs ?? getReaderModeRetryDelayMs();
