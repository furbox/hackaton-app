<script lang="ts">
    ...
    let { session } = $props<{ session: LayoutSession }>();

    const user = $derived(session?.user ?? null);
    const isAuthenticated = $derived(Boolean(user));
    const displayName = $derived(user?.username ?? 'Guest');
            ...
            <div class="pt-4 pb-3 border-t border-gray-200 dark:border-gray-800">
                {#if isAuthenticated}
                    <div class="flex items-center px-4">
                        ...
                        <div class="ml-3">
                            <div class="text-base font-medium text-gray-800 dark:text-white">{displayName}</div>
                        </div>
                    </div>
